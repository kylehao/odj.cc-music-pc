    ...
    <script type="text/javascript" charset="utf-8">
        layui.use(['table','form','layer'], function(){
          ...
          var layer = layui.layer;
            table.render({
                elem: '#table',//表格id
                url:"./admin.php?a=getusers",//list接口地址
          ...
          table.on('tool(table)', function(obj){
              if(obj.event === 'del'){
                  layer.confirm('真的删除吗', function(index){
                      $.post("./admin.php?a=admin_delete",{email:obj.data.userPrincipalName,id:obj.data.id},function(res){
                        if (res.code == 0) {
                            obj.del();//删除表格这行数据
                        }
                        layer.msg(res.msg);
              ...
              if(obj.event === 'accountactive'){
                  layer.confirm('允许登录?', function(index){
                      $.post("./admin.php?a=invitation_code_activeaccount",{email:obj.data.userPrincipalName},function(res){
                       if (res.code == 1) {
                          layer.closeAll();
              ...
              if(obj.event === 'setuserasadminbyid'){
                  layer.confirm('设为管理?', function(index){
                      $.post("./admin.php?a=invitation_code_setuserasadminbyid",{id:obj.data.id},function(res){
                       if (res.code == 1) {
                          layer.closeAll();
                          layui.use('table', function(){
                              var table = layui.table;
			  ...
			  if(obj.event === 'deluserasadminbyid'){
                  layer.confirm('取消管理?', function(index){
                      $.post("./admin.php?a=invitation_code_deluserasadminbyid",{id:obj.data.id},function(res){
                       if (res.code == 1) {
                          layer.closeAll();
                          layui.use('table', function(){
                              var table = layui.table;
              ...
              if(obj.event === 'accountinactive'){
                  layer.confirm('禁止登录?', function(index){
                      $.post("./admin.php?a=invitation_code_inactiveaccount",{email:obj.data.userPrincipalName},function(res){
                       if (res.code == 1) {
                          layer.closeAll();
                          layui.use('table', function(){
                              var table = layui.table;
			...
			$('#change_account').click(function(){
                  layer.confirm('确认切换全局?', function(index){
                      ...
                      $.post("./admin.php?a=changeaccount",{account:accountid},function(res){
                       if (res.code == 1) {
                          layer.closeAll();
                        }
                        layer.msg(res.msg);
